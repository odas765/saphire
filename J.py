...
async def process_track_metadata(track_id, r_id, cover=None, thumbnail=False):
    ...
    if t_meta.get('VERSION'):
        metadata['title'] += f' ({t_meta["VERSION"]})'  # fixed
...
async def process_album_metadata(album_id: int, a_meta: dict, t_meta: list, r_id):
    ...
    if a_meta.get('VERSION'):
        metadata['title'] += f' ({a_meta["VERSION"]})'  # fixed
...
def get_artists_name(meta: dict):
    artists = [a['ART_NAME'] for a in meta['ARTISTS']]
